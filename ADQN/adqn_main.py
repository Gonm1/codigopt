...
if __name__ == '__main__':
    ...
    agent = Agent(gamma=0.99, epsilon=1.0, alpha=0.00025,
                  input_dims=env.observation_space.shape, n_actions=env.action_space.n, mem_size=25_000,
                  eps_min=0.1, batch_size=32, replace=10000, eps_dec=1e-5, 
                  save_name='adqn_model', load_name='dqn_model_5000it.h5', K=10)
    ...
    for episode in tqdm(range(num_games)):
        ...
        if n_steps % test_every_frames == 0 or episode == num_games - 1:
            ...
            df.to_csv(f"./csv/adqnTestData_PerEpisode{n_test - 1}.csv", sep=',',index=False)

            df = pandas.DataFrame(data={"Test_instance": [n_test_instance], "Score_avg": [reward_avg], "Value_avg": [value_avg], "N_steps": [n_steps]})
            df.to_csv(f"./csv/adqnTestData_{n_test_instance}.csv", sep=',',index=False)
            n_test_instance += 1
            ...
            df.to_csv(f"./csv/adqnTrainingData-{episode+1}.csv", sep=',',index=False)
