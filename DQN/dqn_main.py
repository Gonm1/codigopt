...
if __name__ == '__main__':
    ...
    for episode in tqdm(range(num_games)):
        ...
        if n_steps % test_every_frames == 0 or episode == num_games - 1:
            ...
            df.to_csv(f"./csv/dqnTestData_PerEpisode{n_test - 1}.csv", sep=',',index=False)

            df = pandas.DataFrame(data={"Test_instance": [n_test_instance], "Score_avg": [reward_avg], "Value_avg": [value_avg], "N_steps": [n_steps]})
            df.to_csv(f"./csv/dqnTestData_{n_test_instance}.csv", sep=',',index=False)
            n_test_instance += 1
            ...
            df.to_csv(f"./csv/dqnTrainingData-{episode+1}.csv", sep=',',index=False)
